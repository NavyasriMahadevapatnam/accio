...
const BrowseLost = ({ token }) => {
  ...
  const fetchItems = async () => {
    try {
      const response = await axios.get(`${BACKEND_URL}/api/lost/list`);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-slate-900 mb-4 font-outfit">Lost Items</h1>
          <Input
        ...
        {loading ? (
          ...
          <div data-testid="empty-state" className="text-center py-20">
            <p className="text-slate-600 text-lg">No lost items found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map((item) => (
              <Link key={item.id} to={`/item/lost/${item.id}`}>
                <div data-testid="item-card" className="group bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer">
                  ...
                  <div className="p-6">
                    ...
                    <Button className="w-full mt-4 bg-secondary text-white hover:bg-secondary/90 h-10 rounded-full">View Details</Button>
                  </div>
                </div>
              </Link>
            ))}
